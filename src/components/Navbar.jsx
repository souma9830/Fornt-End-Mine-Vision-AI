...
const Navbar = () => {
  ...
  return (
    <>
      {isMobileMenuOpen && (
        <div
          className="fixed inset-0 z-40 transition-opacity duration-500 bg-black/30 backdrop-blur-md lg:hidden"
          onClick={() => setIsMobileMenuOpen(false)}
        />
      )}

      <nav
        className={`fixed top-5 left-5 right-5 z-50 w-[calc(100%-2.5rem)] rounded-3xl shadow-lg transition-all duration-700 backdrop-filter backdrop-blur-2xl ${
          isScrolled
            ? "bg-gradient-to-r from-gray-100 via-gray-50 to-blue-50/90 py-2 border border-gray-200"
            : "bg-gradient-to-r from-gray-50/80 via-white/70 to-blue-50/70 py-4 border border-gray-100/70"
        }`}
        style={{
          boxShadow:
            "0 8px 25px rgba(0, 0, 0, 0.08), inset 0 0 30px rgba(255, 255, 255, 0.4)",
        }}
      >
        <div className="container flex items-center justify-between px-8 mx-auto">
          {/* Logo */}
          <div className="flex items-center space-x-3 cursor-default select-none">
            <div className="relative h-12 w-14">
              <div className="absolute inset-0 rounded-full bg-gradient-to-r from-blue-300 to-cyan-300 blur-xl opacity-60 animate-pulse-slow"></div>
              <img
                src={logo}
                alt="MineVision AI Logo"
                className="relative z-10 object-contain h-12 w-14 transition-transform duration-300 ease-in-out hover:scale-110"
                draggable={false}
                loading="eager"
              />
            </div>
            <h1 className="text-2xl font-extrabold tracking-wide text-transparent select-text bg-clip-text bg-gradient-to-r from-blue-600 to-cyan-500 drop-shadow-sm">
              MineVision.<span className="text-cyan-500">AI</span>
            </h1>
          </div>
          ...
          <ul className="hidden space-x-10 font-semibold tracking-wide text-gray-800 select-none lg:flex">
            {["home", "features", "about", "contact"].map((item) => (
              <li key={item} className="relative group">
                ...
                <span
                  className={`absolute bottom-0 left-0 w-0 h-1 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-full transition-all duration-300 group-hover:w-full ${
                    activeLink === item ? "w-full" : ""
                  }`}
                />
              </li>
          ...
          <div className="items-center hidden lg:flex">
            <button
              className="relative z-20 px-5 py-2 text-sm font-bold text-white transition-transform transform rounded-full shadow-lg bg-gradient-to-r from-blue-500 to-cyan-400 hover:scale-105 focus:outline-none focus:ring-4 focus:ring-cyan-300/50"
              aria-label="Login"
            >
          ...
          {/* Mobile Menu Button */}
          <button
            className="text-gray-700 transition-transform duration-300 lg:hidden focus:outline-none"
            onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
            aria-label="Toggle menu"
            aria-expanded={isMobileMenuOpen}
          >
        ...
        <div
          ...
          aria-hidden={!isMobileMenuOpen}
        >
          <div className="px-8 py-6 mt-4 text-gray-700 border shadow-inner rounded-2xl bg-gradient-to-b from-white/90 to-blue-50/90 backdrop-blur-lg border-gray-200">
            <ul className="flex flex-col space-y-5 font-semibold tracking-wide">
              {["home", "features", "about", "contact"].map((item) => (
                <li key={item}>
                  <a
                    ...
                    className={`block w-full py-3 px-4 rounded-lg border-l-4 transition-all duration-300 ${
                      activeLink === item
                        ? "text-cyan-600 border-cyan-500 bg-cyan-100"
                        : "text-gray-700 border-transparent hover:text-cyan-600 hover:border-cyan-400/40"
                    }`}
                  >
                    {item.charAt(0).toUpperCase() + item.slice(1)}
                  </a>
              ...
              <li className="pt-4 mt-4 border-t border-gray-200">
                <button
                  className="relative w-full px-5 py-2 text-sm font-bold text-white transition-transform transform rounded-full shadow-lg bg-gradient-to-r from-blue-500 to-cyan-400 hover:scale-105 focus:outline-none focus:ring-4 focus:ring-cyan-300/50"
                  aria-label="Login"
                >
                  <span className="relative z-10">Login</span>
                  <span className="absolute inset-0 bg-white rounded-full opacity-10 blur animate-pulse-slow"></span>
